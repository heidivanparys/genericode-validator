...
<schema
    ...
    <pattern>
        <rule context="SimpleCodeList">
            <assert
                id="rule_1"
                test="count(/*/ColumnSet/Key)>0">Rule 1 - A code list must have at least one key, unless it is a metadata-only definition without a SimpleCodeList element.</assert>
        </rule>
        ...
        <rule context="CanonicalUri">
            <assert
                id="rules_4_25_30_48"
                test="matches(.,'^\w+:')"
                diagnostics="nodeValue">Rules 4, 25, 30, 48 - A canonical URI must be an absolute URI, it must not be relative.</assert>
        </rule>
        <rule context="CanonicalVersionUri">
            <assert
                id="rules_6_27_32_44_50"
                test="matches(.,'^\w+:')"
                diagnostics="nodeValue">Rules 6, 27, 32, 44, 50 - A canonical version URI must be an absolute URI, it must not be relative.</assert>
        </rule>
        ...
        <rule context="Data/@Type">
            <assert
                id="rule_19"
                test="not(contains(.,':'))"
                diagnostics="nodeValue">Rule 19 - A datatype ID must not include a namespace prefix.</assert>
        </rule>
        <rule context="@ExternalRef">
            <assert
                id="rule_24"
                test="not(starts-with(.,'#'))"
                diagnostics="nodeValue">Rule 24 - The external reference must not be prefixed with a '#' symbol.</assert>
        </rule>
        ...
        <rule context="ShortName">
            <assert
                id="rule_39"
                test="not(matches(.,'\s'))"
                diagnostics="nodeValue">Rule 39 - A short name must not contain whitespace characters.</assert>
        </rule>
        <rule context="ComplexValue">
            <assert
                ...
                test="every $name in */local-name(.) satisfies
                    ...
                    ('*',$name)">Rule
                42 - The names of all direct child elements of the 'ComplexValue' element must match the datatype ID for the matching column, unless that ID is set to '*' (&lt;Row>
                <value-of select="1 + count( ancestor::Row/preceding-sibling::Row)" />
                ) expecting:
                <value-of select='for $id in ancestor::Value[1]/@ColumnRef return
                    normalize-space($availableColumns[@Id=$id][1]/Data/@Type)' />
            </assert>
            <assert
                id="rule_43"
         ...
         ('*',$uri)">Rule
                43 - The namespace URIs of all direct child elements of the 'ComplexValue' element must match the datatype library URI for the matching column, unless that URI is set to '*' (&lt;Row>
                <value-of select="1 + count( ancestor::Row/preceding-sibling::Row)" />
                ) expecting:
                <value-of select='for $id in ancestor::Value[1]/@ColumnRef return
                    normalize-space($availableColumns[@Id=$id][1]/Data/@DatatypeLibrary)' />
        ...
        <rule context="Row">
            <let
                name="thisRowsIds"
                value="Value/@ColumnRef" />

            <assert
                id="rule_37"
                test="not(some $id in $requiredColumns/@Id satisfies not( $id = $thisRowsIds ))">Rule
                37 - A value must be provided for each required column (&lt;Row>
                <value-of select="1 + count( preceding-sibling::Row)" />
                ):
                <value-of select="$requiredColumns/@Id[not(.=$thisRowsIds)]" />
            </assert>

      <!--Miscellaneous integrity rules-->
      <!--any column can have only one definition in a given row-->
            <assert test="not(some $ref in Value/@ColumnRef satisfies $ref = ( Value except $ref/.. )/@ColumnRef)">Duplicate
                column references not allowed in a given row (&lt;Row>
                <value-of select="1 + count( preceding-sibling::Row)" />
                ):
                <value-of select='for $dup in distinct-values( Value/@ColumnRef/string(.) ) return
                  if( count(Value/@ColumnRef[. = $dup]) > 1 )
                  then concat("ColumnRef=""",$dup,"""") else ()' />
            </assert>
